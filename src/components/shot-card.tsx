...
export function ShotCard({ shot, shotIndex, isHoleCompleted, endDistance }: ShotCardProps) {
  ...
  // Format initial distance: green values in feet, others in yards.
  ...
  const handleDistanceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Don't dispatch if input is empty or just a decimal point
    if (!isNaN(newDistNum) && newDist !== '' && !newDist.endsWith('.')) {
  ...
  const shotDistanceValue = shot.startDistance - endDistance;
  const distanceUnit = shot.lie === 'Green' ? 'ft' : 'yds';
  const displayDistance = shot.lie === 'Green' ? (shot.startDistance * 3).toFixed(1) : shot.startDistance.toFixed(0);
